...
> **What this is**: zero-diff fork of `huggingface/ml-intern` that ships the **cosmos-lab** library — **9 NEW agents (6 Cosmos-specialty for ML lifecycle + 3 governance for the meta layer) + ~16 production governance infrastructure components**, all built on **ml-intern's tool primitives** (agent_loop blocks, 16 generic tools, sandbox, MCP, cost estimation) **leveraged AS-IS, not reimplemented**. The 6 specialty agents (DataAgent, EvalAgent, TrainOrchestrator, OptimizeAgent, MultimodalPipelineAgent, CodeAgent) do real Cosmos-specialized ML lifecycle work. The 3 governance agents (GepaOptimizer, CapabilityProbe, CrossAgentEvaluator) self-improve, red-team, and cross-compare. Deployed via `nat run cosmos-lab.yaml` into NVIDIA Cosmos team's stack. See `docs/01_north_star.md` for vision; PLAN_V2.md §0.6 + §0.65 + §0.9 for v6 thesis (current revision).
